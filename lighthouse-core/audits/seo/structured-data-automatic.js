  ...
  static async audit(artifacts) {
    ...
    await Promise.all(
      artifacts.JsonLD.map(async (aaaa, idx) => {
        ...
        Object.keys(errorsByCode).forEach(content => {
          const errors = errorsByCode[content];
          /** @type {LH.Audit.Details.Snippet['lineMessages']} */
          const lineMessages = []
          /** @type {LH.Audit.Details.Snippet['generalMessages']} */
          ...
          // console.log(node);
          // note: should not be tabledata, listdataor sth
          tableData.push(
            Audit.makeSnippetDetails({
        ...
        if (errors.length === 0) {
          tableData.push(Audit.makeSnippetDetails({
            content: JSON.stringify(JSON.parse(jsonLD), null, 2),
            // todo: how does i18n work?
            title,
            lineMessages: [],
            generalMessages: [],
